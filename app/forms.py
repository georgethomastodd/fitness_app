...
class Point_goals_form(forms.ModelForm):
    ...
    def clean_goal_start_date(self):
        ...
        for obj in Point_goals.objects.filter(user = this_user):
            if goal_start_date_passed >= obj.goal_start_date and goal_start_date_passed <= obj.goal_end_date: # if the start date is inside preexisting goal
    ...
    def clean_date(self):
        ...
        for obj in User_point_input_model.objects.filter(user = this_user):
            if date == obj.date: # if the start date is inside preexisting goal
                raise forms.ValidationError(("Sorry, there is already an input for this date, you can go update it if you have extra data to input"))
            else:
                pass
